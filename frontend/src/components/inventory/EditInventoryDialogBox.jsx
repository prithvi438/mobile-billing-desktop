...
import {
    ...
    FiLayers, FiAlertTriangle, FiTag, FiEdit2, FiInfo
} from "react-icons/fi";
...
const EditInventoryDialogBox = ({ open, item, onClose, onSuccess }) => {
    ...
    useEffect(() => {
        ...
        setForm({
            ...
            sku: item.sku ?? "",
            barcodes: Array.isArray(item.barcodes) ? item.barcodes.join(", ") : "",
    ...
    const submit = async () => {
        if (!form.name || !form.category || !form.costPrice || !form.sellingPrice) {
            setInfoDialog({
                ...
                message: "Please fill all mandatory fields (marked with *) before updating.",
                type: "warning",
            });
            return;
        }
        ...
        try {
            ...
            await axios.patch(`${API_BASE_URL}/inventory/${item._id}`, payload, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            setInfoDialog({
                open: true,
                title: "Update Failed",
                message: err.response?.data?.message || "Something went wrong while updating item.",
                type: "error",
            });
        } finally {
            setLoading(false);
                ...
                <div style={{ ...styles.modal, animation: "modalPop 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)" }}>
                    ...
                    <div style={styles.header}>
                        <div style={styles.headerTextWrapper}>
                            <div style={styles.iconCircle}>
                                <FiEdit2 size={22} color={BRAND.primary} />
                            </div>
                            <div>
                                <h2 style={styles.title}>Edit Inventory Item</h2>
                                <p style={styles.subtitle}>Update product specifications & stock details</p>
                            </div>
                        </div>
                    ...
                    <div style={styles.grid}>
                            ...
                            <div style={styles.card}>
                                ...
                                <div style={styles.row}>
                                    <Field label="SKU Code" style={{ flex: 1 }}>
                                        <input style={styles.input} value={form.sku} placeholder="Auto-gen if empty" onChange={(e) => setForm({ ...form, sku: e.target.value })} />
                                    </Field>
                        ...
                        <div style={styles.column}>
                            ...
                            <div style={styles.card}>
                                <div style={styles.row}>
                                    <Field label="Cost Price *" style={{ flex: 1 }}>
                                        <div style={styles.inputWithIcon}>
                                            ...
                                            <input type="number" style={styles.borderlessInput} value={form.costPrice} placeholder="0.00" onChange={(e) => setForm({ ...form, costPrice: e.target.value })} />
                                        </div>
                                    </Field>
                                    <Field label="Selling Price *" style={{ flex: 1 }}>
                                        <div style={styles.inputWithIcon}>
                                            <FiDollarSign size={14} color={BRAND.primary} />
                                            <input type="number" style={styles.borderlessInput} value={form.sellingPrice} placeholder="0.00" onChange={(e) => setForm({ ...form, sellingPrice: e.target.value })} />
                                        </div>
                                    </Field>
                                </div>
                                ...
                                <div style={styles.row}>
                                    <Field label="Tax (GST %)" style={{ flex: 1 }}>
                                        <div style={styles.inputWithIcon}>
                                            ...
                                            <input type="number" style={styles.borderlessInput} value={form.gstRate} placeholder="0" onChange={(e) => setForm({ ...form, gstRate: e.target.value })} />
                                        </div>
                                    </Field>
                                    <Field label="Current Stock *" style={{ flex: 1 }}>
                                        <div style={styles.inputWithIcon}>
                                            <FiLayers size={14} color={BRAND.textMuted} />
                                            <input type="number" style={styles.borderlessInput} value={form.quantity} placeholder="Qty" onChange={(e) => setForm({ ...form, quantity: e.target.value })} />
                                        </div>
                                    </Field>
                                </div>
                                ...
                                <Field label="Low Stock Alert">
                                    <div style={styles.inputWithIcon}>
                                        ...
                                        <input type="number" style={styles.borderlessInput} value={form.lowStockAlert} placeholder="Notify when below..." onChange={(e) => setForm({ ...form, lowStockAlert: e.target.value })} />
                                    </div>
                                </Field>
                                ...
                                <Field label="Barcodes">
                                    <div style={styles.inputWithIcon}>
                                        ...
                                        <input style={styles.borderlessInput} value={form.barcodes} placeholder="Comma separated barcodes" onChange={(e) => setForm({ ...form, barcodes: e.target.value })} />
                                    </div>
                                </Field>
                            </div>
                            ...
                            <div style={styles.infoBox}>
                                ...
                                <span>Note: Changing stock quantity manually here will create an inventory adjustment log.</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={styles.footer}>
                        ...
                        <button style={styles.primaryBtn} onClick={submit} disabled={loading}>
                            <FiSave size={18} /> {loading ? "Updating..." : "Update Product"}
                        </button>
                    </div>
                </div>
            </div>
...
const styles = {
    ...
    unitToggle: { display: "flex", background: "#F1F5F9", padding: 4, borderRadius: 12, border: `1px solid ${BRAND.border}`, width: '100%' },
    unitBtn: { flex: 1, border: "none", padding: "8px 0", borderRadius: 8, fontSize: 11, fontWeight: 800, cursor: "pointer", transition: "0.2s" },
    infoBox: {
        marginTop: 12, padding: "12px 16px", borderRadius: "14px", background: "#F0F9FF",
...
export default EditInventoryDialogBox;